...
class CnnMFCC(nn.Module):
    def __init__(self, n_classes:int, sample_rate:int = 16_000, max_audio_length_seconds:int=5, dropout:float=0.5):
        ...
        linear_in = self.conv(self.feature_extractor(torch.ones(1, 1, sample_rate*max_audio_length_seconds))).shape[-1]
